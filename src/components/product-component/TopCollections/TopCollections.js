...
//     topCollectionsProductsByHighlightsItems
// } from './TopCollectionsMockData';
...
export default class TopCollections extends Component {
    constructor() {
        ...
        this.state = {
            ...
            // topCollectionsProductsByHighlights: topCollectionsProductsByHighlightsItems,
            productHighlights: [],
            topCollectionsProductsByHighlights: [],
            id: 0
        }
    }
    ...
    componentDidMount() {
        ...
        .then((res) => {
            console.log('productHighlightsInProTopColl', res.data.data);
            this.setState({ productHighlights: res.data.data });

            let topCollection = this.state.productHighlights.find((highlight) => {
                return highlight.productHighlightCode === "VPH01" ? highlight : null;
            });
            console.log('topCollectionInProTopColl', topCollection);

            if(topCollection) {
                fetchProductsByProductHighlightId(topCollection.storeId, topCollection.id)
                .then((res) => {
                    console.log('topCollectionsProductsByHighlightsIDWithWebAddonInProTopColl', res.data.data);
                    this.setState({ topCollectionsProductsByHighlights: res.data.data });
                })
                .catch((err) => {
                    console.log(err);
                })
    ...
    handleRightClick = () => {
        document.getElementsByClassName('topCollectionsItems')[0].scrollLeft += 275;
    }
    ...
    render() {
        return (
            <div className="topCollectionsContainer">
                {/* ITERATING TO GET TOP COLLECTIONS HIGHLIGHTS */}
                {this.state.productHighlights.map((highlight, index) => {
                    return highlight.productHighlightCode === "VPH01" ?
                        <div className="topCollectionsHeader" key={index}>
                            <p>{highlight.productHighlight}</p>
                            <p>{highlight.description}</p>
                        </div>
                    : null
                })}
                <div className="topCollectionsViewAll">
                    <p>View All</p>
                </div>
                <div className="topCollectionsItems">
                    {/* ITERATING OVER TOP COLLECTIONS PRODUCTS BY HIGHLIGHTS */}
                    {this.state.topCollectionsProductsByHighlights.map((product, index) => {
                        return (
                            <div key={index} className="topCollectionsItem" onMouseOver={() => this.handleOnMouseOver(product.productId)}>

                                {/* Product Top - Small Image & Ratings */}
                                <div className="topCollectionsProductTop">
                                    <div className="topCollectionsProductThumbImage">
                                        <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}/${product.varients[0].image[1].media.media}`} alt="noImage" />
                                    </div>
                                    <div className="topCollectionsProductRatings">
                                        <ul className="">
                                            <li className="">
                                ...
                                {/* Product Big Image */}
                                <div className="topCollectionsProductImage">
                                    <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}/${product.varients[0].image[1].media.media}`} alt="noImage" />
                                </div>

                                {/* Product Name, Description, Price & Sales Price */}
                                <div className="topCollectionsProductInfo">
                                    <p className="topCollectionsProductName">
                                        {product.productName}
                                        <span className="topCollectionsBrandName">{}</span>
                                    </p>
                                    <p className="topCollectionsProductDescription">
                                        {product.shortDescription}
                                    </p>
                                    <p className="topCollectionsProductPrice">
                                        AED <span>{product.varients[0].price}</span>
                                    </p>
                                    <p className="topCollectionsProductSalesPrice">
                                        AED <span>{product.varients[0].salesPrice}</span>
                                    </p>
                                </div>

                                {/* Product Bottom - On Hover Icons */}
                                <div className="topCollectionsProductBottom" style={{ display: this.state.id !== product.productId ? `none` : `grid` }}>
                                    <div className="topCollectionsProductViewDetail">
                                    ...
                                    <div className="topCollectionsProductShoppingBag">
                                    ...
                                    <div className="topCollectionsProductWishlist">
                ...
                <div className="topCollectionsLeftArrow" onClick={() => this.handleLeftClick()}>
                    <img src={vpLeftArrow} alt="noImage" height="16" />
                </div>
                <div className="topCollectionsRightArrow" onClick={() => this.handleRightClick()}>
