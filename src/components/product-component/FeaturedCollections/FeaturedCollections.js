...
// import {
//     productHighlightsItems,
//     featuredProductsByHighlightsItems
// } from './FeaturedCollectionsMockData';
...
class FeaturedCollections extends Component {
    ...
    constructor() {
        super();
        this.state = {
            // productHighlights: productHighlightsItems,
            // featuredProductsByHighlights: featuredProductsByHighlightsItems,
            productHighlights: [],
            featuredProductsByHighlights: []
        }
    }
    ...
    componentDidMount() {
        ...
        .then((res) => {
            console.log('featuredProductsByHighlightsInProFeaturedProducts', res.data.data);
            this.setState({ productHighlights: res.data.data });

            let featuredProduct = this.state.productHighlights.find((highlight) => {
                return highlight.productHighlightCode === "VPH02" ? highlight : null;
            });
            console.log('featuredProductInProFeaturedProducts', featuredProduct);

            if(featuredProduct) {
                fetchProductsByProductHighlightId(featuredProduct.storeId, featuredProduct.id)
                .then((res) => {
                    console.log('featuredProductsByHighlightsIDWithWebAddonInProFeaturedProducts', res.data.data);
                    this.setState({ featuredProductsByHighlights: res.data.data });
                })
            }
        })
        .catch((err) => {
    ...
    handleRightClick = () => {
        document.getElementsByClassName('featuredCollectionsItems')[0].scrollLeft += 275;
    }
    ...
    render() {
        return (
            <div className="featuredCollectionsContainer">
                {/* ITERATING TO GET FEATURED PRODUCTS HIGHLIGHTS */}
                {this.state.productHighlights.map((highlight, index) => {
                    return highlight.productHighlightCode === "VPH02" ?
                        <div className="featuredCollectionsHeader" key={index}>
                            <p>{highlight.productHighlight}</p>
                            <p>{highlight.description}</p>
                        </div>
                    : null
                })}
                <div className="featuredCollectionsViewAll">
                    <p>View All</p>
                </div>
                <div className="featuredCollectionsItems">
                    {/* ITERATING OVER FEATURED PRODUCTS BY HIGHLIGHTS */}
                    {this.state.featuredProductsByHighlights.map((product, index) => {
                        return (
                            <div key={index} className="featuredCollectionsItem" onMouseOver={() => this.handleOnMouseOver(product.productId)}>

                                {/* Product Top - Small Image & Ratings */}
                                <div className="featuredCollectionsProductTop">
                                    <div className="featuredCollectionsProductThumbImage">
                                        <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}/${product.varients[0].image[1].media.media}`} alt="noImage" />
                                    </div>
                                    <div className="featuredCollectionsProductRatings">
                                        <ul className="">
                                            <li className="">
                                ...
                                {/* Product Big Image */}
                                <div className="featuredCollectionsProductImage">
                                    <img src={`${process.env.REACT_APP_IMAGE_BASE_URL}/${product.varients[0].image[1].media.media}`} alt="noImage" />
                                </div>

                                {/* Product Name, Description, Price & Sales Price */}
                                <div className="featuredCollectionsProductInfo">
                                    <p className="featuredCollectionsProductName">
                                        {product.productName}
                                        <span className="featuredCollectionsBrandName">{}</span>
                                    </p>
                                    <p className="featuredCollectionsProductDescription">
                                        {product.shortDescription}
                                    </p>
                                    <p className="featuredCollectionsProductPrice">
                                        AED <span>{product.varients[0].price}</span>
                                    </p>
                                    <p className="featuredCollectionsProductSalesPrice">
                                        AED <span>{product.varients[0].salesPrice}</span>
                                    </p>
                                </div>

                                {/* Product Bottom - On Hover Icons */}
                                <div className="featuredCollectionsProductBottom" style={{ display: this.state.id !== product.productId ? `none` : `grid` }}>
                                    <div className="featuredCollectionsProductViewDetail">
                                    ...
                                    <div className="featuredCollectionsProductShoppingBag">
                                    ...
                                    <div className="featuredCollectionsProductWishlist">
                ...
                <div className="featuredCollectionsLeftArrow" onClick={() => this.handleLeftClick()}>
                    <img src={vpLeftArrow} alt="noImage" height="16" />
                </div>
                <div className="featuredCollectionsRightArrow" onClick={() => this.handleRightClick()}>
